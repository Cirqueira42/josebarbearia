...
-- Drop ALL existing policies (use exact names from current state)
...
CREATE POLICY "Anyone can view barbers" ON barbers AS PERMISSIVE FOR SELECT TO anon, authenticated USING (true);
...
CREATE POLICY "Anyone can view blocked slots" ON blocked_slots AS PERMISSIVE FOR SELECT TO anon, authenticated USING (true);
...
CREATE POLICY "Services are viewable by everyone" ON services AS PERMISSIVE FOR SELECT TO anon, authenticated USING (true);
